...
# --------------------------------------------------------------------------
    ...
    while (z < n):                                    # condition to check if we have obtained ZFS or not.
        ...
        for i in range(len(Potential_A)):
            v = Potential_A[i];
            DZ1 = DZ + [v]
            DZ_temp = ZF_Span(A, DZ1);                # Set of nodes that would be colored by including v in a ZFS.
...
# -------------------------------------------------------------------------
    n = A.shape[0];                                                 # no. of nodes in a graph.
    Black = list(S); 
    White = np.setdiff1d(range(n),Black);                           # initialization
