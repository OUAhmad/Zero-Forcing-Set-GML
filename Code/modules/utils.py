...
def preprocess_features_updated(features, yyr, adj):
    """Row-normalize feature matrix and convert to tuple representation"""
    # rowsum = np.array(features.sum(1))
    # r_inv = np.power(rowsum, -1).flatten()
    # r_inv[np.isinf(r_inv)] = 0.
    # r_mat_inv = sp.diags(r_inv)
    # features = r_mat_inv.dot(features).todense()
    features[np.arange(adj.shape[0]), np.sum(adj, axis = 1).reshape(-1)] = 1
    features[:, -1] = yyr.reshape((-1, 1))
...
def distances(support):
    laplacian =  sp.csc_matrix((support[-1][1], (support[-1][0][:, 0], support[-1][0][:, 1])), shape=support[-1][2]).todense()
    adj = np.zeros(laplacian.shape)
    adj[laplacian<0] = 1
