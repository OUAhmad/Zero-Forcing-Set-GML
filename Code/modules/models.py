...
class GCN_DEEP_DIVER(Model):
    ...
    def _loss(self):
        ...
        elif FLAGS.loss == 'Dist_CE_loss':
        # 32 outputs
            diver_loss = my_softmax_cross_entropy_updated(self.outputs[:,0:self.output_dim], self.placeholders['labels'], self.distance)
            for i in range(1,FLAGS.diver_num):
                diver_loss = tf.reduce_min([diver_loss, my_softmax_cross_entropy_updated(self.outputs[:, 2*i:2*i + self.output_dim], self.placeholders['labels'], self.distance)])
            self.loss += diver_loss

        elif FLAGS.loss == 'Dist_CE_Soln_loss':
        # 32 outputs
            diver_loss = my_softmax_cross_entropy_updated_new(self.outputs[:,0:self.output_dim], self.placeholders['labels'], self.distance, self.solution_nodes)
            for i in range(1,FLAGS.diver_num):
                diver_loss = tf.reduce_min([diver_loss, my_softmax_cross_entropy_updated_new(self.outputs[:, 2*i:2*i + self.output_dim], self.placeholders['labels'], self.distance, self.solution_nodes)])
            self.loss += diver_loss
